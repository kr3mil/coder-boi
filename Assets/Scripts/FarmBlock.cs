...
public class FarmBlock : MonoBehaviour
{
  ...
  public Sprite[] SeedTextures = new Sprite[4];
  ...
  public bool Plant()
  {
    if (!isPlanted)
    {
      seedRenderer.sprite = SeedTextures[0];
  ...
  private void Grow()
  {
    if (Random.Range(0, chanceToGrow) == 0)
    {
      ...
      if (Growth > 99)
      {
        seedRenderer.sprite = SeedTextures[3];
        Growth = 100;
      }
      else if (Growth > 66)
      {
        seedRenderer.sprite = SeedTextures[2];
      }
      else if (Growth > 33)
      {
        seedRenderer.sprite = SeedTextures[1];
      }
    }
  }
}